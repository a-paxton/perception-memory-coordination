...
```{r print-sanity-check, invisible=TRUE, echo=TRUE}

cat('Problematic rows identified: ',dim(sanity_df)[1],sep='')
...
cat('Total usable dyads included: ',dim(usable_dyads)[1], sep='')
...
```{r print-quesionnaire-dyads, invisible=TRUE, echo=TRUE}
...
```{r plot-training_slopes, invisible=TRUE, echo=TRUE, warning = FALSE, error = FALSE, message = FALSE}
...
```{r print-likely-outliers, invisible=TRUE, echo=TRUE}
...
# replace hugely off-base numbers with missing data and then remove all missing
winnowed_info_df = winnowed_info_df %>%
...
```{r plot-normalized-error-over-all-trials, invisible=TRUE, echo=TRUE, warning = FALSE, error = FALSE, message = FALSE}
