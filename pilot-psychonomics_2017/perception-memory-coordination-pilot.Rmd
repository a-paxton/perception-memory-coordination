---
title: "Interpersonal coordination in perception and memory: Pilot analyses"
...
* `./data-pilot/`: Contains experimental data. All data for included dyads are freely available in the OSF repository for the project: `https://osf.io/8fu7x/`.
...
experiment_files = list.dirs('./data-pilot', recursive=FALSE)
...
ggsave(plot = training_slope_plot,
       height = 3,
       width = 5,
       filename = './figures-pilot/pmc-training_slopes.jpg')
...
ggsave(plot = training_slope_plot,
       ...
       width = 5,
       dpi=100,
       filename = './figures-pilot/pmc-training_slopes-knitr.jpg')

```

![**Figure**. Included participants' absolute normalized error over all training trials and best-fit line (in blue).](./figures-pilot/pmc-training_slopes-knitr.jpg)
...
ggsave(plot = normalized_error_hist,
       height = 4,
       width = 5,
       filename = './figures-pilot/pmc-error_hist.jpg')
...
ggsave(plot = normalized_error_hist,
       ...
       width = 5,
       dpi=100,
       filename = './figures-pilot/pmc-error_hist-knitr.jpg')

```

![**Figure**. Histogram of individual participants' normalized error for each guess over all trials. Histogram is further broken down by the trial number at which each guess was given.](./figures-pilot/pmc-error_hist-knitr.jpg)
...
write.table(winnowed_info_df, './data-pilot/winnowed_data.csv', sep=',',
...
# read in dataset
winnowed_info_df = read.table('./data-pilot/winnowed_data.csv', sep=',',header = TRUE)
...
ggsave(plot = all_variable_plot,
       height = 4,
       width = 8,
       filename = './figures-pilot/pmc-all_variables.jpg')
...
ggsave(plot = all_variable_plot,
       ...
       width = 8,
       dpi=100,
       filename = './figures-pilot/pmc-all_variables-knitr.jpg')

```

![**Figure**. Density plots of questionnaire responses, normalized error, and improvement over training trials.](./figures-pilot/pmc-all_variables-knitr.jpg)
...
* **Do partners' ratings of their own and their partners' trustworthiness influence the dynamics of perceptual coordination?** No.

We also have some ideas that will shape future analyses.

* When we have more time:
    * *Do partners' ratings of their own and their partners' cooperativity influence the dynamics of perceptual coordination?*
...
# plot normalized error of the first guess submitted on each trial
first_guess_error = first_guess_plot %>%
  
  # dummy-code training as above or below/at mean
  mutate(training_improvement = factor((training_improvement > mean(training_improvement))*1)) %>%

  # create the plot, adding each guess as jitter
  ggplot(., aes(x = trial_number,
                y = normalized_error,
                color = training_improvement)) +
  geom_jitter(width=.1,
              height=.05,
              alpha=.5) +
  ...
  xlab('Trial') +
  ggtitle('Normalized error\nof first guess by trial\nand training improvement')

# save a high-resolution version of the plot
ggsave(plot = first_guess_error,
       height = 4,
       width = 3,
       filename = './figures-pilot/pmc-first_guess.jpg')
...
ggsave(plot = first_guess_error,
       ...
       width = 3,
       dpi=100,
       filename = './figures-pilot/pmc-first_guess-knitr.jpg')

```

![**Figure**. Density plots of questionnaire responses, normalized error, and improvement over training trials. Truncated to not display high-error guesses, using a cutoff of absolute normalized error of 0.20.](./figures-pilot/pmc-all_variables-knitr.jpg)
...
cross_corr_plot = ggplot(ccf_df, aes(x = lag,
                              y = r)) +
...
ggsave(plot = cross_corr_plot,
       height = 4,
       width = 5,
       filename = './figures-pilot/pmc-cross_corr_plot.jpg')
...
ggsave(plot = cross_corr_plot,
       ...
       width = 5,
       dpi=100,
       filename = './figures-pilot/pmc-cross_corr_plot.jpg-knitr.jpg')

```

![**Figure**. Cross-correlation of partners' normalized error across turns, overlaid with best-fit line.](./figures-pilot/pmc-cross_corr_plot.jpg-knitr.jpg)
